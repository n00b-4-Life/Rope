...
class GUI(tk.Tk):
    def __init__(self, models):
        ...
        self.target_face = {
            ...
            'AssignedEmbedding': [],
            # the currently assigned source embedding, including averaged ones
        }
        self.target_faces = []
    ...
    def create_gui(self):
        ...
        # Buttons
        self.widget['StartButton'] = GE.Button(self.layer['topleft'], 'StartRope', 1, self.load_all, None, 'control',
                                               10, 9, width=200)
        self.widget['OutputFolderButton'] = GE.Button(self.layer['topleft'], 'OutputFolder', 1,
                                                      self.select_save_video_path, None, 'control', x=240, y=1,
                                                      width=190)
        self.output_videos_text = GE.Text(self.layer['topleft'], '', 1, 240, 20, 190, 20)
        self.selected_video_text = GE.Text(self.layer['topleft'], '', 1, 700, 20, 400, 20)
        # Right
        ...
        self.control['ClearVramButton'] = GE.Button(self.layer['topright'], 'ClearVramButton', 1, self.clear_mem, None,
                                                    'control', x=5, y=9, width=85, height=20)
        ...
        # Buttons
        self.widget['VideoFolderButton'] = GE.Button(frame, 'LoadTVideos', 2, self.select_video_path, None, 'control',
                                                     10, 1, width=195)
        self.input_videos_text = GE.Text(frame, '', 2, 10, 20, 190, 20)

        # Input Videos Canvas
        self.target_media_canvas = tk.Canvas(self.layer['InputVideoFrame'], style.canvas_frame_label_3, height=100,
                                             width=195)
        ...
        self.target_media_canvas.bind("<Button-3>", lambda event: self.save_image())
        self.target_media_canvas.create_text(8, 20, anchor='w', fill='grey25', font=("Arial italic", 20),
                                             text=" Input Videos")
        ...
        # Buttons
        self.widget['FacesFolderButton'] = GE.Button(frame, 'LoadSFaces', 2, self.select_faces_path, None, 'control',
                                                     10, 1, width=195)
        self.input_faces_text = GE.Text(frame, '', 2, 10, 20, 190, 20)

        # Scroll Canvas
        self.source_faces_canvas = tk.Canvas(self.layer['InputVideoFrame'], style.canvas_frame_label_3, height=100,
                                             width=195)
        ...
        self.source_faces_canvas.bind("<Button-3>", lambda event: self.save_image())
        self.source_faces_canvas.create_text(8, 20, anchor='w', fill='grey25', font=("Arial italic", 20),
                                             text=" Input Faces")
        ...
        self.widget['AudioButton'] = GE.Button(frame, 'Audio', 2, self.toggle_audio, None, 'control', x=0, y=0,
                                               width=100)

        frame = tk.Frame(preview_data, style.canvas_frame_label_2, height=24, width=100)
        frame.grid(row=0, column=1)
        self.widget['MaskViewButton'] = GE.Button(frame, 'MaskView', 2, self.toggle_maskview, None, 'control', x=0, y=0,
                                                  width=100)

        frame = tk.Frame(preview_data, style.canvas_frame_label_2, height=24, width=200)
        frame.grid(row=0, column=2)
        self.widget['PreviewModeTextSel'] = GE.TextSelection(frame, 'PreviewModeTextSel', '', 2, self.set_view, True,
                                                             'control', width=200, height=20, x=0, y=0, text_percent=1)
        ...
        self.video.bind("<ButtonRelease-1>", lambda event: save_image())
        # self.video.bind("<ButtonRelease-1>", lambda event: self.toggle_play_video())
        self.video.bind("<Button-3>", lambda event: self.save_image())
        ...
        # Left Side
        self.layer['play_controls_left'] = tk.Frame(self.layer['preview_frame'], style.canvas_frame_label_2, height=30,
                                                    width=100)
        self.layer['play_controls_left'].grid(row=0, column=0, sticky='NEWS', pady=0)
        self.widget['SaveImageButton'] = GE.Button(self.layer['play_controls_left'], 'SaveImageButton', 2,
                                                   self.save_image, None, 'control', x=10, y=5, width=100)
        ...
        column = 0
        col_delta = 50
        self.widget['TLBegButton'] = GE.Button(play_control_frame, 'TLBeginning', 2, self.preview_control, 'q',
                                               'control', x=column, y=2, width=20)
        column += col_delta
        self.widget['TLLeftButton'] = GE.Button(play_control_frame, 'TLLeft', 2, self.preview_control, 'a', 'control',
                                                x=column, y=2, width=20)
        column += col_delta
        self.widget['TLRecButton'] = GE.Button(play_control_frame, 'Record', 2, self.toggle_rec_video, None, 'control',
                                               x=column, y=2, width=20)
        column += col_delta
        self.widget['TLPlayButton'] = GE.Button(play_control_frame, 'Play', 2, self.toggle_play_video, None, 'control',
                                                x=column, y=2, width=20)
        column += col_delta
        self.widget['TLRightButton'] = GE.Button(play_control_frame, 'TLRight', 2, self.preview_control, 'd', 'control',
                                                 x=column, y=2, width=20)
        ...
        self.widget['AddMarkerButton'] = GE.Button(right_playframe, 'AddMarkerButton', 2, self.update_marker, 'add',
                                                   'control', x=0, y=5, width=20)
        self.widget['DelMarkerButton'] = GE.Button(right_playframe, 'DelMarkerButton', 2, self.update_marker, 'delete',
                                                   'control', x=25, y=5, width=20)
        self.widget['PrevMarkerButton'] = GE.Button(right_playframe, 'PrevMarkerButton', 2, self.update_marker, 'prev',
                                                    'control', x=50, y=5, width=20)
        self.widget['NextMarkerButton'] = GE.Button(right_playframe, 'NextMarkerButton', 2, self.update_marker, 'next',
                                                    'control', x=75, y=5, width=20)
        ...
        self.widget['SaveImageButton'] = GE.Button(self.layer['image_controls'], 'SaveImageButton', 2, self.save_image,
                                                   None, 'control', x=10, y=5, width=100)
        self.widget['AutoSwapButton'] = GE.Button(self.layer['image_controls'], 'AutoSwapButton', 2,
                                                  self.toggle_auto_swap, None, 'control', x=150, y=5, width=100)
        ...
        self.widget['FindFacesButton'] = GE.Button(button_frame, 'FindFaces', 2, self.find_faces, None, 'control', x=0,
                                                   y=0, width=112, height=33)
        self.widget['ClearFacesButton'] = GE.Button(button_frame, 'ClearFaces', 2, self.clear_faces, None, 'control',
                                                    x=0, y=33, width=112, height=33)
        self.widget['SwapFacesButton'] = GE.Button(button_frame, 'SwapFaces', 2, self.toggle_swapper, None, 'control',
                                                   x=0, y=66, width=112, height=33)
        ...
        self.found_faces_canvas.bind("<Button-3>", lambda event: self.save_image())
        self.found_faces_canvas.create_text(8, 45, anchor='w', fill='grey25', font=("Arial italic", 20),
                                            text=" Found Faces")
        ...
        self.widget['DelEmbedButton'] = GE.Button(button_frame, 'DelEmbed', 2, self.print_selected_faces, None, 'control', x=0, y=0,
                                                  width=112, height=33)
        ...
        self.merged_embeddings_text.bind("<Return>",
                                         lambda event: self.save_selected_source_faces(self.merged_embedding_name))
        ...
        self.merged_faces_canvas.bind("<MouseWheel>",
                                      lambda event: self.merged_faces_canvas.xview_scroll(-int(event.delta / 120.0),
                                                                                          "units"))
        self.merged_faces_canvas.bind("<Button-3>", lambda event: self.save_image())
        self.merged_faces_canvas.create_text(8, 45, anchor='w', fill='grey25', font=("Arial italic", 20),
                                             text=" Merged Faces")
        ...
        parameters_control_frame = tk.Frame(self.layer['parameter_frame'], style.canvas_frame_label_2, bd=0,
        ...
        frame.grid(row=0, column=0)
        self.widget['SaveParamsButton'] = GE.Button(frame, 'SaveParamsButton', 2, self.parameter_io, 'save', 'control',
                                                    x=0, y=8, width=100)

        frame = tk.Frame(parameters_control_frame, style.canvas_frame_label_2, height=42, width=100)
        frame.grid(row=0, column=1)
        self.widget['LoadParamsButton'] = GE.Button(frame, 'LoadParamsButton', 2, self.parameter_io, 'load', 'control',
                                                    x=0, y=8, width=100)

        frame = tk.Frame(parameters_control_frame, style.canvas_frame_label_2, height=42, width=100)
        frame.grid(row=0, column=2)
        self.widget['DefaultParamsButton'] = GE.Button(frame, 'DefaultParamsButton', 2, self.parameter_io, 'default',
                                                       'control', x=0, y=8, width=100)

        self.layer['parameters_canvas'] = tk.Canvas(self.layer['parameter_frame'], style.canvas_frame_label_3, bd=0,
                                                    width=width)
        self.layer['parameters_canvas'].grid(row=1, column=0, sticky='NEWS', pady=0, padx=0)

        self.layer['parameters_frame'] = tk.Frame(self.layer['parameters_canvas'], style.canvas_frame_label_3, bd=0,
        ...
        self.layer['parameter_scroll_canvas'] = tk.Canvas(self.layer['parameter_frame'], style.canvas_frame_label_3,
                                                          bd=0, )
        ...
        self.static_widget['parameters_scrollbar'] = GE.Scrollbar_y(self.layer['parameter_scroll_canvas'],
                                                                    self.layer['parameters_canvas'])
        ...
        # Restore
        self.widget['RestorerSwitch'] = GE.Switch2(self.layer['parameters_frame'], 'RestorerSwitch', 'Restorer', 3,
                                                   self.update_data, 'parameter', 398, 20, 1, row)
        row += switch_delta
        self.widget['RestorerTypeTextSel'] = GE.TextSelection(self.layer['parameters_frame'], 'RestorerTypeTextSel',
                                                              'Restorer Type', 3, self.update_data, 'parameter',
                                                              'parameter', 398, 20, 1, row, 0.62)
        row += row_delta
        self.widget['RestorerDetTypeTextSel'] = GE.TextSelection(self.layer['parameters_frame'],
                                                                 'RestorerDetTypeTextSel', 'Detection Alignment', 3,
                                                                 self.update_data, 'parameter', 'parameter', 398, 20, 1,
                                                                 row, 0.62)
        row += row_delta
        self.widget['RestorerSlider'] = GE.Slider2(self.layer['parameters_frame'], 'RestorerSlider', 'Blend', 3,
                                                   self.update_data, 'parameter', 398, 20, 1, row, 0.62)
        row += top_border_delta
        self.static_widget['9'] = GE.Separator_x(self.layer['parameters_frame'], 0, row)
        row += bottom_border_delta

        # Threshhold
        self.widget['ThresholdSlider'] = GE.Slider2(self.layer['parameters_frame'], 'ThresholdSlider',
                                                    'Similarity Threshhold', 3, self.update_data, 'parameter', 398, 20,
                                                    1, row, 0.62)
        row += top_border_delta
        ...
        # Orientation
        self.widget['OrientSwitch'] = GE.Switch2(self.layer['parameters_frame'], 'OrientSwitch', 'Orientation', 3,
                                                 self.update_data, 'parameter', 398, 20, 1, row)
        row += switch_delta
        self.widget['OrientSlider'] = GE.Slider2(self.layer['parameters_frame'], 'OrientSlider', 'Angle', 3,
                                                 self.update_data, 'parameter', 398, 20, 1, row, 0.62)
        row += top_border_delta
        ...
        # Strength
        self.widget['StrengthSwitch'] = GE.Switch2(self.layer['parameters_frame'], 'StrengthSwitch', 'Strength', 3,
                                                   self.update_data, 'parameter', 398, 20, 1, row)
        row += switch_delta
        self.widget['StrengthSlider'] = GE.Slider2(self.layer['parameters_frame'], 'StrengthSlider', 'Amount', 3,
                                                   self.update_data, 'parameter', 398, 20, 1, row, 0.62)
        row += top_border_delta
        ...
        # Border
        self.widget['BorderTopSlider'] = GE.Slider2(self.layer['parameters_frame'], 'BorderTopSlider',
                                                    'Top Border Distance', 3, self.update_data, 'parameter', 398, 20, 1,
                                                    row, 0.62)
        row += row_delta
        self.widget['BorderSidesSlider'] = GE.Slider2(self.layer['parameters_frame'], 'BorderSidesSlider',
                                                      'Sides Border Distance', 3, self.update_data, 'parameter', 398,
                                                      20, 1, row, 0.62)
        row += row_delta
        self.widget['BorderBottomSlider'] = GE.Slider2(self.layer['parameters_frame'], 'BorderBottomSlider',
                                                       'Bottom Border Distance', 3, self.update_data, 'parameter', 398,
                                                       20, 1, row, 0.62)
        row += row_delta
        self.widget['BorderBlurSlider'] = GE.Slider2(self.layer['parameters_frame'], 'BorderBlurSlider', 'Border Blend',
                                                     3, self.update_data, 'parameter', 398, 20, 1, row, 0.62)
        row += top_border_delta
        self.static_widget['7'] = GE.Separator_x(self.layer['parameters_frame'], 0, row)
        row += bottom_border_delta

        # Diff
        self.widget['DiffSwitch'] = GE.Switch2(self.layer['parameters_frame'], 'DiffSwitch', 'Differencing', 3,
                                               self.update_data, 'parameter', 398, 20, 1, row)
        row += switch_delta
        self.widget['DiffSlider'] = GE.Slider2(self.layer['parameters_frame'], 'DiffSlider', 'Amount', 3,
                                               self.update_data, 'parameter', 398, 20, 1, row, 0.62)
        row += top_border_delta
        ...
        # Occluder
        self.widget['OccluderSwitch'] = GE.Switch2(self.layer['parameters_frame'], 'OccluderSwitch', 'Occluder', 3,
                                                   self.update_data, 'parameter', 398, 20, 1, row)
        row += switch_delta
        self.widget['OccluderSlider'] = GE.Slider2(self.layer['parameters_frame'], 'OccluderSlider', 'Size', 3,
                                                   self.update_data, 'parameter', 398, 20, 1, row, 0.62)
        row += top_border_delta
        ...
        # FaceParser - Face
        self.widget['FaceParserSwitch'] = GE.Switch2(self.layer['parameters_frame'], 'FaceParserSwitch', 'Face Parser',
                                                     3, self.update_data, 'parameter', 398, 20, 1, row)
        row += switch_delta
        self.widget['FaceParserSlider'] = GE.Slider2(self.layer['parameters_frame'], 'FaceParserSlider', 'Background',
                                                     3, self.update_data, 'parameter', 398, 20, 1, row, 0.62)
        row += row_delta
        self.widget['MouthParserSlider'] = GE.Slider2(self.layer['parameters_frame'], 'MouthParserSlider', 'Mouth', 3,
                                                      self.update_data, 'parameter', 398, 20, 1, row, 0.62)
        row += top_border_delta
        ...
        # Blur
        self.widget['BlendSlider'] = GE.Slider2(self.layer['parameters_frame'], 'BlendSlider', 'Overall Mask Blend', 3,
                                                self.update_data, 'parameter', 398, 20, 1, row, 0.62)
        row += top_border_delta
        ...
        # Color Adjustments
        self.widget['ColorSwitch'] = GE.Switch2(self.layer['parameters_frame'], 'ColorSwitch', 'Color Adjustments', 3,
                                                self.update_data, 'parameter', 398, 20, 1, row)

        self.widget['HSVSwitch'] = GE.Switch2(self.layer['parameters_frame'], 'HSVSwitch', 'HSV Mode', 3,
                                              self.update_data, 'parameter',
                                              199, 20, 200, row)

        row += switch_delta
        self.widget['ColorRedSlider'] = GE.Slider2(self.layer['parameters_frame'], 'ColorRedSlider', 'Red', 3,
                                                   self.update_data, 'parameter', 398, 20, 1, row, 0.62)
        row += row_delta
        self.widget['ColorGreenSlider'] = GE.Slider2(self.layer['parameters_frame'], 'ColorGreenSlider', 'Green', 3,
                                                     self.update_data, 'parameter', 398, 20, 1, row, 0.62)
        row += row_delta
        self.widget['ColorBlueSlider'] = GE.Slider2(self.layer['parameters_frame'], 'ColorBlueSlider', 'Blue', 3,
                                                    self.update_data, 'parameter', 398, 20, 1, row, 0.62)
        row += row_delta
        self.widget['ColorGammaSlider'] = GE.Slider2(self.layer['parameters_frame'], 'ColorGammaSlider', 'Gamma', 3,
                                                     self.update_data, 'parameter', 398, 20, 1, row, 0.62)
        row += top_border_delta
        ...
        self.widget['FaceAdjSwitch'] = GE.Switch2(self.layer['parameters_frame'], 'FaceAdjSwitch',
                                                  'Input Face Adjustments', 3, self.update_data, 'parameter', 398, 20,
                                                  1, row)
        row += switch_delta
        self.widget['KPSXSlider'] = GE.Slider2(self.layer['parameters_frame'], 'KPSXSlider', 'KPS - X', 3,
                                               self.update_data, 'parameter', 398, 20, 1, row, 0.62)
        row += row_delta
        self.widget['KPSYSlider'] = GE.Slider2(self.layer['parameters_frame'], 'KPSYSlider', 'KPS - Y', 3,
                                               self.update_data, 'parameter', 398, 20, 1, row, 0.62)
        row += row_delta
        self.widget['KPSScaleSlider'] = GE.Slider2(self.layer['parameters_frame'], 'KPSScaleSlider', 'KPS - Scale', 3,
                                                   self.update_data, 'parameter', 398, 20, 1, row, 0.62)
        row += row_delta
        self.widget['FaceScaleSlider'] = GE.Slider2(self.layer['parameters_frame'], 'FaceScaleSlider', 'Face Scale', 3,
                                                    self.update_data, 'parameter', 398, 20, 1, row, 0.62)
        row += top_border_delta
        ...
        self.widget['SwapperTypeTextSel'] = GE.TextSelection(self.layer['parameters_frame'], 'SwapperTypeTextSel',
                                                             'Swapper Resolution', 3, self.update_data, 'parameter',
                                                             'parameter', 398, 20, 1, row, 0.62)
        row += row_delta

        self.widget['DetectScoreSlider'] = GE.Slider2(self.layer['parameters_frame'], 'DetectScoreSlider',
                                                      'Detect Score', 3, self.update_data, 'parameter', 398, 20, 1, row,
                                                      0.62)
        row += row_delta

        self.widget['MergeTextSel'] = GE.TextSelection(self.layer['parameters_frame'], 'MergeTextSel', 'Merge Math', 3,
                                                       self.select_input_faces, 'merge', '', 398, 20, 1, row, 0.62)
        row += row_delta

        self.widget['DetectTypeTextSel'] = GE.TextSelection(self.layer['parameters_frame'], 'DetectTypeTextSel',
                                                            'Detection Type', 3, self.update_data, 'parameter',
                                                            'parameter', 398, 20, 1, row, 0.62)
        row += row_delta

        self.widget['RecordTypeTextSel'] = GE.TextSelection(self.layer['parameters_frame'], 'RecordTypeTextSel',
                                                            'Record Type', 3, self.update_data, 'parameter',
                                                            'parameter', 398, 20, 1, row, 0.62)
        row += row_delta
        self.widget['VideoQualSlider'] = GE.Slider2(self.layer['parameters_frame'], 'VideoQualSlider', 'FFMPEG Quality',
                                                    3, self.update_data, 'parameter', 398, 20, 1, row, 0.62)
        row += row_delta

        self.widget['ThreadsSlider'] = GE.Slider2(self.layer['parameters_frame'], 'ThreadsSlider', 'Threads', 3,
                                                  self.update_data, 'parameter', 398, 20, 1, row, 0.62)
        row += row_delta
        ...
        # CLIP
        self.widget['CLIPSwitch'] = GE.Switch2(self.layer['parameters_frame'], 'CLIPSwitch', 'Text-Based Masking', 3,
                                               self.update_data, 'parameter', 398, 20, 1, row)
        row += switch_delta
        self.widget['CLIPTextEntry'] = GE.Text_Entry(self.layer['parameters_frame'], 'CLIPTextEntry',
                                                     'Text-Based Masking', 3, self.update_data, 'parameter', 398, 20, 1,
                                                     row, 0.62)
        row += row_delta
        self.widget['CLIPSlider'] = GE.Slider2(self.layer['parameters_frame'], 'CLIPSlider', 'Amount', 3,
                                               self.update_data, 'parameter', 398, 20, 1, row, 0.62)
        row += top_border_delta
        self.static_widget['12'] = GE.Separator_x(self.layer['parameters_frame'], 0, row)
        ...
        self.layer['tooltip_label'] = tk.Label(self.layer['tooltip_frame'], style.info_label, wraplength=width - 10,
                                               image=self.blank, compound='left', height=80, width=width - 10)
        self.layer['tooltip_label'].place(x=5, y=5)
        self.static_widget['13'] = GE.Separator_x(self.layer['tooltip_frame'], 0, 0)
        ######### FaceLab

        self.layer['facelab_canvas'] = tk.Canvas(self.layer['parameter_frame'], style.canvas_frame_label_3, bd=0,
                                                 width=width)
        self.layer['facelab_canvas'].grid(row=1, column=0, sticky='NEWS', pady=0, padx=0)
        #
        self.layer['facelab_frame'] = tk.Frame(self.layer['facelab_canvas'], style.canvas_frame_label_3, bd=0,
                                               width=width, height=11000)
        ...
        #
        self.layer['facelab_scroll_canvas'] = tk.Canvas(self.layer['parameter_frame'], style.canvas_frame_label_3,
                                                        bd=0, )
        ...
        self.static_widget['facelab_scrollbar'] = GE.Scrollbar_y(self.layer['facelab_scroll_canvas'],
                                                                 self.layer['facelab_canvas'])
        # row_delta = 20
        # row = 1
        # for i in range(512):
        ...
        self.status_left_label = tk.Label(bottom_frame, style.donate_1, cursor="hand2",
                                          text=" Questions/Help/Discussions (Discord)")
        ...
        self.donate_label = tk.Label(bottom_frame, style.donate_1, text="Enjoy Rope? Please Support! (Paypal) ",
                                     anchor='e')
        self.donate_label.grid(row=0, column=2, sticky='NEWS')
        self.donate_label.bind("<Button-1>",
                               lambda e: self.callback("https://www.paypal.com/donate/?hosted_button_id=Y5SB9LSXFGRF2"))
        ...
        if self.focus_get() != self.widget['CLIPTextEntry'] and self.focus_get() != self.merged_embeddings_text:
            ...
            if self.video_loaded:
                ...
                if event == ' ':
                    ...
                    self.update_marker('next')
                elif event == 'p':
                    self.update_marker('prev')
    ...
    def initialize_gui(self):
        ...
        self.geometry('%dx%d+%d+%d' % (
            self.json_dict['dock_win_geom'][0], self.json_dict['dock_win_geom'][1], self.json_dict['dock_win_geom'][2],
        ...
        self.bind("<Button-2>", lambda event: self.update_marker('add'))
        self.bind("<Control-Button-2>", lambda event: self.update_marker('delete'))
    ...
    def load_input_faces(self, sort_faces=True):
        self.source_faces = []
        ...
        try:
            ...
            with open("merged_embeddings.txt", "r") as embedfile:
                ...
                for line in temp:
                    # Split the line into name and embeddings
                    if line.startswith("Name:"):
                        identifier, embedding_str = line.split(", ", 1)
                        name = identifier[6:]  # Extract the name after "Name: "
                        embeddings = np.array(list(map(float, embedding_str.split())), dtype='float32')
                        temp0.append([name, embeddings])
            ...
            for j in range(len(temp0)):
                ...
                self.source_faces[j]["TKButton"] = tk.Button(self.merged_faces_canvas, style.media_button_off_3,
                                                             image=self.blank, text=temp0[j][0], height=14, width=84,
                                                             compound='left')

                self.source_faces[j]["TKButton"].bind("<ButtonRelease-1>",
                                                      lambda event, arg=j: self.select_input_faces(event, arg))
                self.source_faces[j]["TKButton"].bind("<MouseWheel>",
                                                      lambda event: self.merged_faces_canvas.xview_scroll(
                                                          -int(event.delta / 120.0), "units"))
                self.source_faces[j]["TKButton"].bind("<Button-3>",
                                                      lambda event: self.save_image())
                self.merged_faces_canvas.create_window((j // 4) * 92, 8 + (22 * (j % 4)),
                                                       window=self.source_faces[j]["TKButton"], anchor='nw')
        ...
        # filenames = [os.path.join(dirpath, f) for (dirpath, dirnames, filenames) in os.walk(directory) for f in
        #             filenames]

        all_files = [os.path.join(dirpath, f) for (dirpath, dirnames, filenames) in os.walk(directory) for f in
                     filenames]
        ...
        for f in all_files:
            ...
            if name not in seen_names and size not in seen_sizes:
        ...
        i = 0
        detected_faces = []  # Store all detected faces with embeddings and scores
            ...
            try:
                ...
                if file_type == 'image':
                    ...
                    if img is not None:
                        ...
                        try:
                            faces_with_scores = self.models.detect_retinaface_with_scores(img, max_num=15, score=0.55)
                        except IndexError:
                            print('No faces detected in the image:', file)
                        else:
                            for face_data in faces_with_scores:
                                kps = face_data["keypoints"]
                                detection_score = face_data["score"]

                                face_emb, cropped_image = self.models.run_recognize(img, kps)
                                ...
                                # Sort detected faces by detection score (highest to lowest)
        ...
        if sort_faces:
            ...
            max_similarity_discard = 99.8
            eliminate_high_similarity = True
            for face in detected_faces:
                matched = False
                for group in groups:
                    if eliminate_high_similarity:
                        discard = False
                        for existing in group:
                            sim = self.findCosineDistance(face["embedding"], existing["embedding"])
                            if sim > max_similarity_discard:
                                print(
                                    f"Eliminated {face['file']} (similarity {sim:.3f}) — too similar to {existing['file']}")
                                discard = True
                                break
                        if discard:
                            matched = True
                    ...
                    # Proceed to similarity grouping only if not discarded
                    sim_to_group_seed = self.findCosineDistance(face["embedding"], group[0]["embedding"])
                    if sim_to_group_seed > similarity_threshold:
                        group.append(face)
                        matched = True
                        break
                if not matched:
            ...
            groups = [[face] for face in detected_faces]
        ...
        if power_sort:
            for group in groups:
                if len(group) <= 1:
                    continue  # No need to sort a single face

                # Compute mean embedding
                embeddings = np.array([face["embedding"] for face in group])
                mean_embedding = np.mean(embeddings, axis=0)

                # Sort faces in group by similarity to mean embedding (descending)
                group.sort(key=lambda face: self.findCosineDistance(face["embedding"], mean_embedding), reverse=True)

        for group_idx, group in enumerate(groups):
            print(f"\nDisplaying Group {group_idx}:")
            for face_idx, face in enumerate(group):
                crop = cv2.cvtColor(face["cropped_image"].cpu().numpy(), cv2.COLOR_BGR2RGB)
                ...
                self.source_faces[-1]["Embedding"] = face["embedding"]
                self.source_faces[-1]["TKButton"] = tk.Button(self.source_faces_canvas,
                                                              style.media_button_off_3,
                                                              image=self.source_faces[-1]["Image"],
                                                              height=90, width=90)
                self.source_faces[-1]["ButtonState"] = False
                self.source_faces[-1]["file"] = face["file"]

                self.source_faces[-1]["TKButton"].bind("<ButtonRelease-1>",
                                                       lambda event, arg=len(self.source_faces) - 1:
                                                       self.select_input_faces(event, arg))
                self.source_faces[-1]["TKButton"].bind("<MouseWheel>", self.source_faces_mouse_wheel)

                x_position = (current_index % 2) * 100
                y_position = (current_index // 2) * 100

                self.source_faces_canvas.create_window(x_position, y_position,
                                                       window=self.source_faces[-1]["TKButton"],
                                                       anchor='nw')
    ...
    def find_faces(self):
        try:
            ...
            print(f"[ERROR] No media selected or error in face detection: {e}")
            return
            ...
            print(f"[DEBUG] {len(ret)} faces detected initially.")

            # Start at threshold 50, incrementing by 5 up to 100
            threshold = 50
            new_faces_found = False

            while threshold <= 100 and not new_faces_found:
                print(f"[DEBUG] Checking faces with threshold: {threshold}")
                new_faces = []
                ...
                if new_faces:
                    ...
                    print(f"[DEBUG] New faces found with threshold: {threshold}")
                    for face in new_faces:
                        ...
                        self.target_faces[last_index]["TKButton"] = tk.Button(
                            self.found_faces_canvas, style.media_button_off_3, height=86, width=86
                        )
                        ...
                        self.target_faces[-1]["TKButton"].config(
                            ...
                            command=lambda k=last_index: self.toggle_found_faces_buttons_state(k)
                        )
                    ...
                    print(f"[DEBUG] No new faces found at threshold: {threshold}, increasing threshold...")
                    threshold += 5
    ...
    def toggle_found_faces_buttons_state(self, button):
        ...
        for i in range(len(self.target_faces[button]["SourceFaceAssignments"])):
            ...
            self.source_faces[self.target_faces[button]["SourceFaceAssignments"][i]]["TKButton"].config(
    ...
    def print_selected_faces(self):

        """Copies selected input faces to a new subdirectory inside 'A:\\rope_exports\\' based on text field input."""

        # Get the directory name from the text field
        dir_name = self.merged_embedding_name.get().strip()
        ...
        image_dir = os.path.join("A:\\rope_exports\\", dir_name)

        # Ensure the directory exists
        os.makedirs(image_dir, exist_ok=True)

        # Get selected files
        selected_files = [face["file"] for face in self.source_faces if face["ButtonState"]]

        if not selected_files:
            print("No input faces selected.")
            return

        # Print selected filenames
        print(f"Copying {len(selected_files)} files to: {image_dir}")
    ...
    def populate_target_videos(self):
        ...
        if self.widget['PreviewModeTextSel'].get() == 'Image':  # images
            for i in range(len(images)):
                self.target_media_buttons.append(
                    tk.Button(self.target_media_canvas, style.media_button_off_3, height=86, width=86))

                rgb_video = Image.fromarray(images[i][0])
                self.target_media.append(ImageTk.PhotoImage(image=rgb_video))
                self.target_media_buttons[i].config(image=self.target_media[i],
                                                    command=lambda i=i: self.load_target(i, images[i][1], self.widget[
                                                        'PreviewModeTextSel'].get()))
                self.target_media_buttons[i].bind("<MouseWheel>", self.target_videos_mouse_wheel)
                self.target_media_buttons[i].bind("<Button-2>", lambda event: self.save_image())
                self.target_media_canvas.create_window((i % 2) * delx, (i // 2) * dely,
                                                       window=self.target_media_buttons[i], anchor='nw')
            ...
            for i in range(len(videos)):
                self.target_media_buttons.append(
                    tk.Button(self.target_media_canvas, style.media_button_off_3, height=65, width=90))
                ...
                self.target_media_buttons[i].bind("<MouseWheel>", self.target_videos_mouse_wheel)
                self.target_media_buttons[i].config(image=self.target_media[i], text=filename, compound='top',
                                                    anchor='n', command=lambda i=i: self.load_target(i, videos[i][1],
                                                                                                     self.widget[
                                                                                                         'PreviewModeTextSel'].get()))
                self.target_media_canvas.create_window((i % 2) * delx, (i // 2) * dely,
                                                       window=self.target_media_buttons[i], anchor='nw')
    ...
    def load_target(self, button, media_file, media_type):
        ...
        if media_type == 'Video':
            ...
            self.selected_video_text.configure(text=os.path.basename(media_file))
        ...
        if temp_holder:
            ...
            if text != "":
                with open("merged_embeddings.txt", "a") as embedfile:
                    ...
                    # Combine identifier and embedding into one line
                    formatted_embedding = ", " + " ".join(map(str, ave_embedding))
                    embedfile.write(f"{identifier}{formatted_embedding}\n")
            else:
    ...
    def update_markers_canvas(self):
        ...
        for marker in self.markers:
            ...
            marker['icon_ref'] = self.layer['markers_canvas'].create_line(position, 0, position, 15,
    ...
    def save_image(self):
        ...
        print('Image saved as:', filename + '.png and .jpg')
    ...
    def parameter_update_from_marker(self, frame):
        ...
        if frame in temp:
            idx = temp.index(frame)  # Get the exact index of the frame in the markers list
            print(f"DEBUG: Frame {frame} matches marker frame {temp[idx]} at index {idx}")
            ...
            desired_keys = {'OrientSwitch', 'OrientSlider', 'MouthParserSlider'}

            # Update only desired keys from the marker parameters
            marker_parameters = self.markers[idx]['parameters']
            for key in desired_keys:
                if key in marker_parameters:
                    self.parameters[key] = copy.deepcopy(marker_parameters[key])

            # Update UI
            for key in desired_keys:
                if key in self.parameters:  # Ensure the key exists in parameters
                    self.widget[key].set(self.parameters[key], request_frame=False)
