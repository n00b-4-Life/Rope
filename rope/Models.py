...
class Models():
    ...
    def run_detect(self, img, detect_mode='Retinaface', max_num=2, score=0.5):
        kpss = []
    ...
    def run_codeformer2(self, image, output):
        ...
        io_binding.bind_input(name='input', device_type='cuda', device_id=0, element_type=np.float32,
                              shape=(1, 3, 1024, 1024), buffer_ptr=image.data_ptr())
        io_binding.bind_output(name='output', device_type='cuda', device_id=0, element_type=np.float32,
                               shape=(1, 3, 1024, 1024), buffer_ptr=output.data_ptr())
    ...
    def detect_retinaface(self, img, max_num, score):
        ...
        kpss = kpss[order,:,:]
        kpss = kpss[keep,:,:]
        #print(f"Final Keypoints After NMS: {kpss}")
        #print(f"Final Scores After NMS: {det[:, 4]}")
        if max_num > 0 and det.shape[0] > max_num:
            ...
            if kpss is not None:
                kpss = kpss[bindex, :]
                #print(f"Keypoints being returned (limited by max_num={max_num}): {kpss}")
        return kpss

    def detect_retinaface_with_scores(self, img, max_num=40, score=0.5):
        """
        Detect faces and return keypoints with scores in a format compatible with load_input_faces.
        """
        # Ensure the RetinaFace model is loaded
        if not self.retinaface_model:
            self.retinaface_model = onnxruntime.InferenceSession('.\models\det_10g.onnx', providers=self.providers)
        ...
        results = [{"keypoints": kp, "score": float(det[i, 4])} for i, kp in enumerate(kpss)]
        return results
    ...
    def recognize(self, img, face_kps):
        ...
        img = v2.Resize((112, 112), interpolation=v2.InterpolationMode.BICUBIC, antialias=False)(img)
